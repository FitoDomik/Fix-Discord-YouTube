...
### 8. Запуск и управление — кратко

- Запуск профиля: двойной клик по нужному `general*.bat`.
- Установка сервиса: `service.bat` → Install Service → выбрать профиль.
- Переключить игровой режим: `service.bat` → Switch Game Filter.
- Включить ipset: `service.bat` → Switch ipset (восстановит `ipset-all.txt` из `.backup`).
- Обновить ipset: `service.bat` → Update ipset list (скачает с GitHub).
